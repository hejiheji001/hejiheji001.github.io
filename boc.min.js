(function(){
	...
	var getCurrentResult = function(){
		var extra = "";
		var completeTasks = Object.values(couponIDs).filter(s => !s.active && s.desc.indexOf("版本") == 0).map(s => [s.desc, Number(s.number), Number(s.price)]);
		if (tasks >= totalTasks){
			extra = "任务完成";
		}else{
			extra = "还在捡漏";
		...
		if(completeTasks.length > 0){
			resultList = completeTasks.reduce((l,r) => [l[0] + "<br/>" + r[0], l[1] + r[1], l[2] + r[2]]);
			...
			$.ajax({
				...
				success: function(r){
					...
					if(d.result == "交易成功" || d.result.indexOf("尚未支付") > -1 || d.result.indexOf("上限") > -1){
						info.active = false;
						tasks = Object.values(couponIDs).filter(s => !s.active).length - 1;
						console.log("OK");		
