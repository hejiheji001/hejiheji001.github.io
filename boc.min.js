			...
			$.ajax({
				...
				success: function(r){
					...
					if(d.result == "交易成功" || d.result.indexOf("尚未支付") > -1 || d.result.indexOf("上限") > -1){
						...
						console.log("OK");		
					}else if(d.result == "活动不存在或未上架" || d.result == "库存不足"){
...
// 			getCoupon("SD");
	...
	var init = function(){
		...
		if(tasks <= totalTasks){
			...
			if($("#run").length == 0){
				$("body").prepend("<p style='position: absolute;left: 0;top: 0;'>正在自动抢购务，必保证屏幕点亮：第<span id='run' style='color:red'>" + runs + "</span>次运行 已经抢购<span id='try' style='color:red'>0<span/>次</p>");
			}else{
				$("#run").html(runs);
			}
