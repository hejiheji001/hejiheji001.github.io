(function(){
	...
	var couponIDs = {
		...
		"WNCoupon": {active: true, ID: "WJHMA000284", price: "70.0", number: "1", desc: "[周四]万宁"},
				...
				success: function(r){
				    ...
				    if(d.stat && d.result){
						if(d.result == "交易成功" || d.result.indexOf("尚未支付") > -1 || d.result.indexOf("上限") > -1){
							...
							delete couponIDs[couponName + "Coupon"];
	...
	var init = function(){
		...
		if($("#run").length == 0){
			$("body").prepend("<p style='position: absolute;left: 0;top: 0; background-color:white'>第<span id='run' style='color:red'>" + runs + "</span>次运行 已抢购<span id='try' style='color:red'>0<span/>次</p>");
		}else{
			$("#run").html(runs);
		}
